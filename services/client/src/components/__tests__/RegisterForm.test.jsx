...
describe('renders', () => {
  ...
  describe('handles form validation correctly', () => {
    ...
    it('when fields are not the proper length', async() => {
      ...
      expect((await findByTestId('errors-username')).innerHTML).toBe('Username must be greater than 5 characters.');
      expect((await findByTestId('errors-email')).innerHTML).toBe('Email must be greater than 5 characters.');
      expect((await findByTestId('errors-password')).innerHTML).toBe('Password must be greater than 10 characters.');
